...
import {
  ...
  ShieldCheck, Clock, PiggyBank, Loader2, Crown, Zap, Globe, Lock, BarChart2,
  CreditCard, Bell, Users, ChevronDown, MessageCircle, Mail, Phone, Send
} from 'lucide-react';
...
const features = [
  { icon: BarChart3, title: 'Dashboard Inteligente', desc: 'Gráficos em tempo real com visão completa de receitas, despesas e saldo.', gradient: 'from-brand-blue to-cyan-500' },
  { icon: Brain, title: 'Finix IA', desc: 'Recomendações personalizadas com inteligência artificial para economizar mais.', gradient: 'from-brand-purple to-pink-500' },
  { icon: Target, title: 'Metas Visuais', desc: 'Acompanhe progresso com barras animadas e alertas de prazo.', gradient: 'from-brand-green to-emerald-500' },
  { icon: Wallet, title: 'Orçamentos Inteligentes', desc: 'Limites por categoria com alertas antes de ultrapassar.', gradient: 'from-amber-500 to-orange-500' },
  { icon: RefreshCw, title: 'Transações Recorrentes', desc: 'Automatize despesas mensais e nunca perca um vencimento.', gradient: 'from-cyan-500 to-brand-blue' },
  { icon: FileDown, title: 'Exportação PDF/Excel', desc: 'Relatórios profissionais para contador ou arquivamento pessoal.', gradient: 'from-rose-500 to-pink-500' },
  { icon: CreditCard, title: 'Gestão de Cartões', desc: 'Controle faturas, limites e parcelamentos em um único lugar.', gradient: 'from-violet-500 to-purple-600' },
  { icon: Bell, title: 'Alertas de Pagamento', desc: 'Notificações de parcelas a vencer para nunca atrasar.', gradient: 'from-orange-500 to-red-500' },
  { icon: BarChart2, title: 'Calendário Financeiro', desc: 'Visualize receitas e despesas por dia em uma view de calendário.', gradient: 'from-teal-500 to-cyan-600' },
];

const faqs = [
  { q: 'O plano gratuito tem limite de tempo?', a: 'O plano FREE dá acesso à dashboard básica. Você pode testar qualquer plano pago por 7 dias sem cartão de crédito.' },
  { q: 'Posso cancelar a qualquer momento?', a: 'Sim. Sem fidelidade e sem taxa de cancelamento. Você cancela com 1 clique nas configurações da conta.' },
  { q: 'Os dados são seguros?', a: 'Sim. Todas as senhas são criptografadas com bcrypt e autenticação via JWT. Seus dados financeiros ficam apenas no banco de dados privado.' },
  { q: 'Funciona para empresas?', a: 'O plano Pro foi desenvolvido para pequenas empresas, com DRE gerencial, centros de custo e suporte prioritário.' },
  { q: 'Posso importar extratos do banco?', a: 'Sim. O Finix suporta importação de arquivos OFX, XLS, CSV e PDF gerados pelos principais bancos brasileiros.' },
  { q: 'A IA é realmente personalizada?', a: 'Sim. O Finix IA analisa seus dados reais para gerar insights específicos para o seu perfil financeiro.' },
];
  ...
  return (
    <div className="min-h-screen bg-white overflow-x-hidden">
      ...
      <section className="relative overflow-hidden">
        <motion.div className="absolute -top-32 -left-32 w-[38rem] h-[38rem] rounded-full blur-3xl opacity-30 hidden md:block"
          style={{ background: 'radial-gradient(circle, #2563EB, transparent 60%)' }}
          animate={{ scale: [1, 1.15, 1], opacity: [0.25, 0.35, 0.25] }}
          transition={{ duration: 8, repeat: Infinity, ease: 'easeInOut' }} />
        <motion.div className="absolute top-10 -right-32 w-[34rem] h-[34rem] rounded-full blur-3xl opacity-30 hidden md:block"
          style={{ background: 'radial-gradient(circle, #7C3AED, transparent 60%)' }}
          animate={{ scale: [1.1, 1, 1.1], opacity: [0.35, 0.25, 0.35] }}
          transition={{ duration: 10, repeat: Infinity, ease: 'easeInOut' }} />
        <motion.div className="absolute bottom-0 left-1/3 w-[28rem] h-[28rem] rounded-full blur-3xl opacity-25 hidden md:block"
          ...
          <div className="order-2 lg:order-1">
            ...
            <motion.h1 initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.05 }}
              ...
              <span className="relative inline-block">
                ...
                <motion.span initial={{ width: 0 }} animate={{ width: '100%' }} transition={{ delay: 0.8, duration: 0.6 }}
            ...
            <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.25 }}
              ...
              <Link to="/register" className="btn-primary !px-6 sm:!px-7 !py-3 sm:!py-3.5 text-sm sm:text-base group w-full sm:w-auto justify-center">
                Começar grátis
            ...
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.4 }}
              ...
              {[
                ...
                <div key={t.text} className="flex items-center gap-1.5">
                  <t.icon className="w-4 h-4 text-brand-green flex-shrink-0" /> {t.text}
                </div>
              ))}
            </motion.div>
      ...
      <section id="features" className="py-16 sm:py-24 bg-gradient-to-b from-white via-slate-50 to-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6">
          <motion.div initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}
            ...
            <div className="chip bg-brand-purple/10 text-brand-purple mb-3 mx-auto border border-brand-purple/20 w-fit">Funcionalidades</div>
          ...
          <div className="mt-16 grid md:grid-cols-3 gap-6">
            {[
              { icon: Lock, title: 'Segurança total', desc: 'Bcrypt + JWT + HTTPS. Seus dados nunca saem do servidor seguro.', badge: 'Seguro', badgeColor: 'bg-green-100 text-green-800' },
              { icon: Zap, title: 'Performance real', desc: 'Backend TypeScript + Prisma + MySQL otimizado para milhares de transações.', badge: 'Rápido', badgeColor: 'bg-blue-100 text-blue-800' },
              { icon: Globe, title: 'Acesse de qualquer lugar', desc: 'Web responsivo, mobile-first. Funciona no celular, tablet e computador.', badge: 'Multiplataforma', badgeColor: 'bg-purple-100 text-purple-800' },
            ].map((item, i) => (
      ...
      <section id="preview" className="py-16 sm:py-24 bg-slate-50 border-y border-slate-100">
        <div className="max-w-7xl mx-auto px-4 sm:px-6">
          ...
          <PreviewCard order={1} badge="Dashboard" title="Seu panorama financeiro em 1 olhada"
            ...
            align="left" visual={<DashboardVisual />} />
          <PreviewCard order={2} badge="Finix IA" title="Análise personalizada em segundos"
            ...
            align="right" visual={<AIInsightsVisual />} />
          <PreviewCard order={3} badge="Metas + Orçamentos" title="Transforme desejos em planos concretos"
            desc="Defina objetivos, acompanhe o progresso com barras animadas e limite gastos por categoria com alertas em tempo real."
            align="left" visual={<GoalsVisual />} />
          <PreviewCard order={4} badge="Calendário Financeiro" title="Visualize o mês inteiro de uma vez"
            desc="Veja cada dia com receitas, despesas e saldo diário. Identifique padrões e picos de gasto instantaneamente."
            align="right" visual={<CalendarVisual />} />
        </div>
      </section>
          ...
          <div className="grid md:grid-cols-3 gap-6 sm:gap-8 max-w-6xl mx-auto items-center">
            ...
            <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: 0.1 }}
              ...
              <div className="flex items-center gap-3 mb-5 mt-4">
                ...
                <div>
                  <h3 className="text-xl font-display font-bold text-white">🚀 Finix Pro</h3>
                  <p className="text-xs text-purple-200">Para pequenas empresas</p>
                </div>
              </div>
              <div className="mb-6">
                <div className="flex items-baseline gap-1">
                  <span className="text-5xl font-display font-extrabold text-white">R$35</span>
                  <span className="text-sm text-purple-200">/mês</span>
                </div>
                <p className="text-xs text-purple-300 mt-1">R$350/ano · economia de R$70</p>
              </div>
        ...
        <div className="relative max-w-4xl mx-auto px-4 sm:px-6 text-center">
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
            ...
            <h2 className="text-3xl sm:text-5xl font-display font-extrabold text-white leading-tight">
              Comece hoje. <br className="hidden sm:block" />Seus dados financeiros merecem isso.
            </h2>
            <p className="mt-5 text-lg text-white/80 max-w-xl mx-auto">
              Junte-se a pessoas que tomaram o controle das próprias finanças com o Finix.
            </p>
            ...
            <p className="mt-5 text-white/60 text-sm">Sem cartão de crédito · Cancele quando quiser</p>
          </motion.div>
        </div>
          ...
          <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}
            ...
            <form onSubmit={handleContactSubmit} className="space-y-4">
              <div className="grid sm:grid-cols-2 gap-4">
                <div>
                  <label className="block text-white/80 text-xs font-semibold mb-1.5">Nome Completo *</label>
                  <input
                    type="text"
                ...
                <div>
                  <label className="block text-white/80 text-xs font-semibold mb-1.5">Email *</label>
                  <input
                    type="email"
              ...
              <div>
                <label className="block text-white/80 text-xs font-semibold mb-1.5">Nome da Empresa</label>
                <input
                  type="text"
              ...
              <div>
                <label className="block text-white/80 text-xs font-semibold mb-1.5">Como podemos ajudar?</label>
                <textarea
              ...
              <p className="text-center text-white/50 text-xs">Ao enviar, você será redirecionado para o WhatsApp</p>
            </form>
          </motion.div>
        </div>
      </section>
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}
            ...
            <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-6">
              ...
              <div className="flex flex-col sm:flex-row gap-3">
                <a href="https://wa.me/5519994737425?text=Olá%20Finix" target="_blank" rel="noreferrer" className="btn-primary inline-flex items-center gap-2">
                  <MessageCircle className="w-4 h-4" /> WhatsApp
                </a>
                <a href="mailto:cvdinizramos@gmail.com" className="btn-outline inline-flex items-center gap-2">
...
const testimonials = [
  { name: 'Rafael Mendes', role: 'Designer · SP', text: 'O Finix mudou meu jogo. Em 3 meses economizei R$ 4.200 só descobrindo para onde meu dinheiro ia.', color: '#2563EB', saving: 'R$ 4.200 economizados' },
  { name: 'Marina Costa', role: 'Engenheira · RJ', text: 'O Finix IA foi impressionante. Identificou que eu gastava demais com delivery e me ajudou a cortar 40%.', color: '#7C3AED', saving: '40% menos em delivery' },
  { name: 'Lucas Almeida', role: 'Dev · BH', text: 'Finalmente um app de finanças bonito e rápido. Os gráficos e as metas me mantêm motivado todo mês.', color: '#22C55E', saving: '3 metas atingidas' },
  { name: 'Patrícia Soares', role: 'Empreendedora · RJ', text: 'A exportação em PDF e Excel salvou meu relatório mensal e facilitou a apresentação ao meu contador.', color: '#F97316', saving: '8h/mês economizadas' },
  { name: 'Guilherme Rocha', role: 'Consultor · SP', text: 'O recurso de metas me fez economizar para uma viagem em apenas 4 meses. Recomendo demais.', color: '#EC4899', saving: 'Viagem em 4 meses' },
];

function TestimonialCarousel() {
  const [current, setCurrent] = useState(0);
  useEffect(() => {
    const timer = window.setInterval(() => setCurrent((prev) => (prev + 1) % testimonials.length), 4500);
  ...
  return (
    <div className="relative max-w-4xl mx-auto">
      <AnimatePresence mode="wait">
        <motion.div key={testimonials[current].name}
...
function HeroDashboard() {
  ...
  return (
    <motion.div initial={{ opacity: 0, scale: 0.9, rotateX: -5 }} animate={{ opacity: 1, scale: 1, rotateX: 0 }}
...
function PreviewCard({ order, badge, title, desc, align, visual }) {
  return (
    <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: '-100px' }}
      ...
      <motion.div whileHover={{ y: -4 }} className="relative perspective">{visual}</motion.div>
    </motion.div>
  );
}
  ...
  return (
    <div className="card !p-5 bg-white" style={{ boxShadow: '0 30px 60px -30px rgba(0,0,0,0.3)' }}>
      ...
      <svg viewBox="0 0 400 120" className="w-full">
        ...
        <motion.path initial={{ pathLength: 0 }} whileInView={{ pathLength: 1 }} viewport={{ once: true }} transition={{ duration: 1.5 }}
          d="M0 80 C 50 70, 90 40, 150 45 S 250 20, 320 15 L 400 10 L 400 120 L 0 120 Z" fill="url(#gLine)" stroke="#22C55E" strokeWidth="2" />
        <motion.path initial={{ pathLength: 0 }} whileInView={{ pathLength: 1 }} viewport={{ once: true }} transition={{ duration: 1.5, delay: 0.3 }}
          d="M0 100 C 60 95, 100 85, 170 88 S 270 70, 340 65 L 400 62 L 400 120 L 0 120 Z" fill="url(#rLine)" stroke="#EF4444" strokeWidth="2" />
      </svg>
    </div>
  );
}
...
function AIInsightsVisual() {
  ...
  return (
    <div className="relative rounded-2xl p-5 border border-brand-purple/20"
      ...
      <div className="grid grid-cols-2 gap-2">
        {items.map((item, idx) => (
          <motion.div key={idx} initial={{ opacity: 0, y: 10 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: idx * 0.15 }}
            className={`rounded-lg border p-2.5 ${colors[item.t]}`}>
            <div className="font-semibold text-[11px]">{item.title}</div>
            <div className="text-[10px] opacity-80 mt-0.5 leading-snug">{item.msg}</div>
          </motion.div>
        ))}
      </div>
    </div>
...
function CalendarVisual() {
  const days = [
    { d: 1, inc: 5000, exp: 0 }, { d: 2, inc: 0, exp: 120 }, { d: 3, inc: 0, exp: 350 },
    { d: 4, inc: 0, exp: 0 }, { d: 5, inc: 0, exp: 89 }, { d: 6, inc: 0, exp: 210 },
    { d: 7, inc: 2000, exp: 1200 }, { d: 8, inc: 0, exp: 45 }, { d: 9, inc: 0, exp: 0 },
    { d: 10, inc: 0, exp: 890 }, { d: 11, inc: 500, exp: 67 }, { d: 12, inc: 0, exp: 340 },
