...
exports.logoutController = exports.refreshTokenController = exports.googleCallbackController = exports.googleRedirectController = void 0;
...
const googleCallbackController = async (req, res) => {
    try {
        ...
        const authResponse = await (0, googleAuthService_1.buildGoogleAuthResponse)(user);
        res.cookie('refresh_token', authResponse.refreshToken, (0, tokenService_1.refreshTokenCookieOptions)(isProduction));
        res.cookie('access_token', authResponse.accessToken, (0, tokenService_1.accessTokenCookieOptions)(isProduction));
        const redirectTo = `${FRONTEND_URL}/oauth-callback?success=true`;
        return res.redirect(redirectTo);
    }
    catch (error) {
        console.error('[OAuth] googleCallbackController error:', error);
        return res.redirect(`${FRONTEND_URL}/oauth-callback?success=false&message=${encodeURIComponent(error.message || 'Erro ao autenticar')}`);
    }
};
...
const refreshTokenController = async (req, res) => {
    try {
        ...
        const accessToken = (0, tokenService_1.createAccessToken)(refreshRecord.user);
        res.cookie('access_token', accessToken, (0, tokenService_1.accessTokenCookieOptions)(isProduction));
        return res.json({ token: accessToken, user: (0, tokenService_1.buildSafeUser)(refreshRecord.user) });
    }
    catch (error) {
