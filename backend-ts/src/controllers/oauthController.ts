...
import { getGoogleAuthUrl, verifyGoogleCode, findOrCreateGoogleUser, buildGoogleAuthResponse } from '../services/googleAuthService';
import { createAccessToken, verifyRefreshToken, revokeRefreshToken, refreshTokenCookieOptions, accessTokenCookieOptions, buildSafeUser } from '../services/tokenService';
...
export const googleCallbackController = async (req: Request, res: Response) => {
  try {
    ...
    res.cookie('refresh_token', authResponse.refreshToken, refreshTokenCookieOptions(isProduction));
    res.cookie('access_token', authResponse.accessToken, accessTokenCookieOptions(isProduction));
...
export const refreshTokenController = async (req: Request, res: Response) => {
  try {
    ...
    res.cookie('access_token', accessToken, accessTokenCookieOptions(isProduction));

    return res.json({ token: accessToken, user: buildSafeUser(refreshRecord.user) });
  } catch (error: any) {
